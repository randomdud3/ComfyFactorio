...
local function get_random_close_spawner(surface, biter_force_name, spawners)
...
local function create_attack_group(surface, force_name, biter_force_name, spawners)
	...
	local spawner = get_random_close_spawner(surface, biter_force_name, spawners)
	if not spawner then
...
Public.main_attack = function()
    ...
    if not global.spawners then
	     global.spawners = surface.find_entities_filtered({type = "unit-spawner", force = biter_force_name})
     end

		set_biter_raffle_table(surface, biter_force_name)

		for c = 1, wave_amount, 1 do
			create_attack_group(surface, force_name, biter_force_name, global.spawners)
		end
